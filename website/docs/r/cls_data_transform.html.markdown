...
* `external_id` - (Optional, String) External ID value, used for cross-account role assumption. Find this in the target account role trust policy.
* `is_cross_account` - (Optional, Bool) Whether the destination topic is in another TencentCloud account. `false`: same account (default); `true`: cross-account.
* `logset_name` - (Optional, String) Name of the logset that contains the destination topic. Used in cross-account scenario.
* `role_arn` - (Optional, String) In the cross-account scenario, the Role ARN value. The target account creates a role for the delivering account. Find this in the target account role list.
* `topic_name` - (Optional, String) Name of the destination topic. Used in cross-account scenario.
