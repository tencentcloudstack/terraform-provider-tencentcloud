...
* `meta_tags` - (Optional, Map) Machine group metadata information list, stored as key-value pairs.
* `ostype` - (Optional, Int) Operating system type. Valid values: 0 (Linux), 1 (Windows).
