...
* `snat_enable` - (Optional, Bool) Whether to enable SNAT (source IP replacement). true: enable, false: disable. Default: false. Note: When SnatEnable is enabled, client source IP will be replaced, and the pass-through client source IP option is disabled, and vice versa.
