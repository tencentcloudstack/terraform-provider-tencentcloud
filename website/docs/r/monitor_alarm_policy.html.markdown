...
The `filter` object of `event_conditions` supports the following:
...
The `hierarchical_notices` object supports the following:

* `notice_id` - (Required, String) Notification template ID.
* `classification` - (Optional, Set) Notification level list, e.g. ["Remind", "Serious"].

The `notice_content_tmpl_bind_infos` object supports the following:

* `content_tmpl_id` - (Required, String) Notification content template ID.
* `notice_id` - (Required, String) Notification template ID.
