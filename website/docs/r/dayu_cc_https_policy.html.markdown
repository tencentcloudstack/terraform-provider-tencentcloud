...
* `resource_type` - (Required, ForceNew) Type of the resource that the CC self-define https policy works for, valid values is `bgpip`.
...
* `exe_mode` - (Optional) Execute mode. Valid values are `alg`, `drop`.
* `switch` - (Optional) Indicate the CC self-define https policy takes effect or not.

The `rule_list` object supports the following:

* `operator` - (Required) Operator of the rule, valid values are `include`, `not_include`, `equal`.
* `skey` - (Required) Key of the rule, valid values are `host`, `cgi`, `ua`, `referer`.
