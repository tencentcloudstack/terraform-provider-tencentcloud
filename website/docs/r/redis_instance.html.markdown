...
* `no_auth` - (Optional, ForceNew) Indicates whether the redis instance support no-auth access. NOTE: Only `vpc_id` specified can set `no_auth` to `true`.
* `password` - (Optional) Password for a Redis user, which should be 8 to 16 characters. NOTE: Only `vpc_id` and `no_auth=true` specified can make password optional.
