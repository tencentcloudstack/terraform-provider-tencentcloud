	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"name": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Event name for data reporting.",
			},
...
func dataSourceTencentCloudRumEventUrlRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_rum_event_url.read")()
	...
	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeRumEventUrlByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		result = result
