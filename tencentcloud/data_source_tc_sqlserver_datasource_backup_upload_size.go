...
func dataSourceTencentCloudSqlserverDatasourceBackupUploadSize() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudSqlserverDatasourceBackupUploadSizeRead,
...
func dataSourceTencentCloudSqlserverDatasourceBackupUploadSizeRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_sqlserver_datasource_backup_upload_size.read")()
	...
	var cosUploadBackupFileSet []*sqlserver.CosUploadBackupFile

	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeSqlserverDatasourceBackupUploadSizeByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
	...
	if cosUploadBackupFileSet != nil {
		for _, cosUploadBackupFile := range cosUploadBackupFileSet {
			...
			ids = append(ids, *cosUploadBackupFile.InstanceId)
