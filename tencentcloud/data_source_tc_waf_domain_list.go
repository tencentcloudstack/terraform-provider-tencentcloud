/*
Use this data source to query detailed information of waf domain_list
...
func dataSourceTencentCloudWafDomainList() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudWafDomainListRead,
...
func dataSourceTencentCloudWafDomainListRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_waf_domain_list.read")()
	...
	if v, ok := d.GetOk("filters"); ok {
		...
		tmpSet := make([]*waf.FiltersItemNew, 0, len(filtersSet))

		for _, item := range filtersSet {
			filtersItemNew := waf.FiltersItemNew{}
			filtersItemNewMap := item.(map[string]interface{})

			if v, ok := filtersItemNewMap["name"]; ok {
				filtersItemNew.Name = helper.String(v.(string))
			}
			if v, ok := filtersItemNewMap["values"]; ok {
				valuesSet := v.(*schema.Set).List()
				filtersItemNew.Values = helper.InterfacesStringsPoint(valuesSet)
			}
			if v, ok := filtersItemNewMap["exact_match"]; ok {
				filtersItemNew.ExactMatch = helper.Bool(v.(bool))
			}
	...
	var domains []*waf.DomainInfo

	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeWafDomainListByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		domains = result
	...
	if domains != nil {
		for _, domainInfo := range domains {
			...
			if domainInfo.CCList != nil {
				domainInfoMap["c_c_list"] = domainInfo.CCList
			}
			...
			if domainInfo.Ports != nil {
				...
				domainInfoMap["ports"] = []interface{}{portsList}
			}
			...
			if domainInfo.LoadBalancerSet != nil {
				...
				domainInfoMap["load_balancer_set"] = []interface{}{loadBalancerSetList}
			}
			...
			if domainInfo.PostCLSStatus != nil {
				domainInfoMap["post_c_l_s_status"] = domainInfo.PostCLSStatus
			}

			if domainInfo.PostCKafkaStatus != nil {
				domainInfoMap["post_c_kafka_status"] = domainInfo.PostCKafkaStatus
			}
