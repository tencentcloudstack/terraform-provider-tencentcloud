/*
Use this data source to query detailed information of ses list_email_identities
...
func dataSourceTencentCloudSesListEmailIdentities() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudSesListEmailIdentitiesRead,
...
func dataSourceTencentCloudSesListEmailIdentitiesRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_ses_list_email_identities.read")()
	...
	var emailIdentities []*ses.EmailIdentity

	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeSesListEmailIdentitiesByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
	...
	ids := make([]string, 0, len(emailIdentities))
	tmpList := make([]map[string]interface{}, 0, len(emailIdentities))

	if emailIdentities != nil {
		for _, emailIdentity := range emailIdentities {
	...
	if maxReputationLevel != nil {
		_ = d.Set("max_reputation_level", maxReputationLevel)
	}

	if maxDailyQuota != nil {
		_ = d.Set("max_daily_quota", maxDailyQuota)
	}
