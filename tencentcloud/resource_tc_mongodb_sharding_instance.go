...
func resourceMongodbShardingInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	request.SecondaryNum = intToPointer(d.Get("nodes_per_shard").(int) - 1)
	request.GoodsNum = intToPointer(1)
	request.InstanceRole = stringToPointer("MASTER")
	request.InstanceType = stringToPointer("SHARD")
...
func resourceMongodbShardingInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("memory", *instance.Memory/1024/(*instance.ReplicationSetNum))
	d.Set("volume", *instance.Volume/1024/(*instance.ReplicationSetNum))
