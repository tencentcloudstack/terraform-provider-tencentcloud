/*
Provides a resource to create a mps process_live_stream
...
mps process_live_stream can be imported using the id, e.g.

```
terraform import tencentcloud_mps_process_live_stream.process_live_stream process_live_stream_id
```
*/
...
func resourceTencentCloudMpsProcessLiveStream() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudMpsProcessLiveStreamCreate,
		Read:   resourceTencentCloudMpsProcessLiveStreamRead,
		Delete: resourceTencentCloudMpsProcessLiveStreamDelete,
		...
		Schema: map[string]*schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"notify_type": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "The notification type, `CMQ` by default. If this parameter is set to `URL`, HTTP callbacks are sent to the URL specified by `NotifyUrl`.&amp;lt;font color=red&amp;gt;Note: If you do not pass this parameter or pass in an empty string, `CMQ` will be used. To use a different notification type, specify this parameter accordingly.&amp;lt;/font&amp;gt;.",
						},
						"notify_url": {
							Type:        schema.TypeString,
							Optional:    true,
			...
			"schedule_id": {
				...
				ForceNew:    true,
				Type:        schema.TypeInt,
				Description: "The scheme ID.Note 1: About `OutputStorage` and `OutputDir`:&amp;amp;lt;li&amp;amp;gt;If an output storage and directory are specified for a subtask of the scheme, those output settings will be applied.&amp;amp;lt;/li&amp;amp;gt;&amp;amp;lt;li&amp;amp;gt;If an output storage and directory are not specified for the subtasks of a scheme, the output parameters passed in the `ProcessMedia` API will be applied.&amp;amp;lt;/li&amp;amp;gt;Note 2: If `TaskNotifyConfig` is specified, the specified settings will be used instead of the default callback settings of the scheme.",
			},
		},
	}
}

func resourceTencentCloudMpsProcessLiveStreamCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_process_live_stream.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s operate mps processLiveStream failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudMpsProcessLiveStreamRead(d, meta)
}

func resourceTencentCloudMpsProcessLiveStreamRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_process_live_stream.read")()
	defer inconsistentCheck(d, meta)()

	return nil
}

func resourceTencentCloudMpsProcessLiveStreamDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_process_live_stream.delete")()
	defer inconsistentCheck(d, meta)()
