	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"total_num": {
				Computed:    true,
				Type:        schema.TypeInt,
				Description: "Total number of entitiesNote: This field may return null, indicating that a valid value cannot be obtained.",
			},
...
func dataSourceTencentCloudCamListEntitiesForPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if list != nil {
		for _, attachEntityOfPolicy := range list {
			...
			ids = append(ids, *attachEntityOfPolicy.Uin)
