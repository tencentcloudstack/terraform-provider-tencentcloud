...
func (me *CfwService) DescribeCfwNatFwSwitchByFilter(ctx context.Context, param map[string]interface{}) (natFwSwitch []*cfw.NatSwitchListData, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = cfw.NewDescribeNatSwitchListRequest()
	)
	...
	for {
		...
		natFwSwitch = append(natFwSwitch, response.Response.Data...)
		if len(response.Response.Data) < int(limit) {
			break
		}
...
func (me *CfwService) DescribeCfwNatfwById(ctx context.Context, natinsId string) (natfw *cfw.NatInstanceInfo, errRet error) {
	logId := getLogId(ctx)
	...
	if len(response.Response.NatInstanceInfo) < 1 {
		return
	}

	natfw = response.Response.NatInstanceInfo[0]
	return
}

func (me *CfwService) DeleteCfwNatfwById(ctx context.Context, natinsId string) (errRet error) {
	logId := getLogId(ctx)
...
func (me *CfwService) CfwNatfwStateRefreshFunc(natinsId string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		ctx := contextNil

		object, err := me.DescribeNatFwInstancesInfo(ctx, natinsId)
...
func (me *CfwService) DescribeCfwNatpolicyById(ctx context.Context, uuid string) (natpolicy *cfw.DescAcItem, errRet error) {
	...
	if len(response.Response.DescAcItem) < 1 {
		return
	}

	natpolicy = response.Response.DescAcItem[0]
	return
}
