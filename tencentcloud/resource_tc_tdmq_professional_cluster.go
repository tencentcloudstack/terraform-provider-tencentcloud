...
func resourceTencentCloudTdmqProfessionalClusterCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_tdmq_professional_cluster.create")()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseTdmqClient().CreateProCluster(request)
		if e != nil {
			return retryError(e)
		} else {
	...
	conf := BuildStateChangeConf([]string{"0"}, []string{"1"}, 8*readRetryTimeout, time.Second, service.TdmqProfessionalClusterStateRefreshFunc(d.Id(), []string{}))
...
func resourceTencentCloudTdmqProfessionalClusterRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_tdmq_professional_cluster.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudTdmqProfessionalClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_tdmq_professional_cluster.update")()
	defer inconsistentCheck(d, meta)()
	...
	if d.HasChange("cluster_name") {
		...
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			result, e := meta.(*TencentCloudClient).apiV3Conn.UseTdmqClient().ModifyCluster(request)
			if e != nil {
				return retryError(e)
			} else {
				log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudTdmqProfessionalClusterDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_tdmq_professional_cluster.delete")()
	defer inconsistentCheck(d, meta)()
