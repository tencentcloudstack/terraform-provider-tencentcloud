	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"event_id": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Add the source of the rule, default to empty.",
			},
			"status": {
				Optional:     true,
				Type:         schema.TypeString,
...
func resourceTencentCloudWafCustomRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	if customRule.Bypass != nil {
		_ = d.Set("bypass", customRule.Bypass)
	}

	if customRule.EventId != nil {
		_ = d.Set("event_id", customRule.EventId)
	}
...
func resourceTencentCloudWafCustomRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	immutableArgs := []string{"domain", "event_id"}
