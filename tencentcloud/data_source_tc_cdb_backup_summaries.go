/*
Use this data source to query detailed information of cdb backup_summaries
...
func dataSourceTencentCloudCdbBackupSummaries() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudCdbBackupSummariesRead,
...
func dataSourceTencentCloudCdbBackupSummariesRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_cdb_backup_summaries.read")()
	...
	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeCdbBackupSummariesByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		totalCount = result
	...
	if items != nil {
		for _, backupSummaryItem := range items {
			...
			ids = append(ids, *backupSummaryItem.IdsHash)
