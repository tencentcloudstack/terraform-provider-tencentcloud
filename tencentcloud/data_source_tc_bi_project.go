	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"data": {
				...
				Description: "Data(Note: This field may return null, indicating that no valid value can be obtained).",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"total": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "Total(Note: This field may return null, indicating that no valid value can be obtained).",
						},
						"total_pages": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "Total pages(Note: This field may return null, indicating that no valid value can be obtained).",
						},
					},
				},
			},
...
func dataSourceTencentCloudBiProjectRead(d *schema.ResourceData, meta interface{}) error {
	...
	if data != nil {
		projectListDataMap := map[string]interface{}{}
				...
				if list.ConfigList != nil {
					...
					for _, configList := range list.ConfigList {
						...
						if configList.Components != nil {
							...
							configListMap["components"] = []interface{}{componentsList}
						}

						configListList = append(configListList, configListMap)
					}

					listMap["config_list"] = []interface{}{configListList}
				}

				listList = append(listList, listMap)
			}

			projectListDataMap["list"] = []interface{}{listList}
		}

		if data.Total != nil {
			projectListDataMap["total"] = data.Total
		}

		if data.TotalPages != nil {
			projectListDataMap["total_pages"] = data.TotalPages
		}

		ids = append(ids, *data.Id)
		_ = d.Set("data", projectListDataMap)
	}
