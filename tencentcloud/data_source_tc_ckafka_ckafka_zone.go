...
func dataSourceTencentCloudCkafkaCkafkaZoneRead(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeCkafkaCkafkaZoneByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		result = result
	...
	if result != nil {
		...
		if result.ZoneList != nil {
			...
			for _, zoneList := range result.ZoneList {
				...
				if zoneList.SalesInfo != nil {
					...
					zoneListMap["sales_info"] = []interface{}{salesInfoList}
				}

				zoneListList = append(zoneListList, zoneListMap)
			}

			zoneResponseMap["zone_list"] = []interface{}{zoneListList}
		}
		...
		if result.ClusterInfo != nil {
			...
			zoneResponseMap["cluster_info"] = []interface{}{clusterInfoList}
		}
		...
		_ = d.Set("result", zoneResponseMap)
	}

	d.SetId(helper.DataResourceIdsHash(ids))
	output, ok := d.GetOk("result_output_file")
	if ok && output.(string) != "" {
		if e := writeToFile(output.(string), zoneResponseMap); e != nil {
			return e
		}
	}
	return nil
