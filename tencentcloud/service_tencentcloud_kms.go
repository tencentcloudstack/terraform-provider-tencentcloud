...
func (me *KmsService) DescribeKmsCloudResourceById(ctx context.Context, keyId string) (cloudResource *kms.KeyMetadata, errRet error) {
	logId := getLogId(ctx)
	...
	if len(response.Response.KeyMetadata) < 1 {
		return
	}

	cloudResource = response.Response.KeyMetadata[0]
	return
}

func (me *KmsService) DeleteKmsCloudResourceById(ctx context.Context, keyId string) (errRet error) {
	logId := getLogId(ctx)
...
func (me *KmsService) DescribeKmsDescribeKeysByFilter(ctx context.Context, param map[string]interface{}) (describeKeys []*kms.KeyMetadata, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = kms.NewDescribeKeysRequest()
	)
	...
	for {
		...
		if response == nil || len(response.Response.KeyMetadata) < 1 {
			break
		}
		describeKeys = append(describeKeys, response.Response.KeyMetadata...)
		if len(response.Response.KeyMetadata) < int(limit) {
			break
		}
...
func (me *KmsService) DescribeKmsDescribeWhiteBoxDecryptKeyByFilter(ctx context.Context, param map[string]interface{}) (describeWhiteBoxDecryptKey []*kms.DescribeWhiteBoxDecryptKeyResponseParams, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = kms.NewDescribeWhiteBoxDecryptKeyRequest()
	)
	...
	for {
		...
		if response == nil || len(response.Response.DecryptKey) < 1 {
			break
		}
		describeWhiteBoxDecryptKey = append(describeWhiteBoxDecryptKey, response.Response.DecryptKey...)
		if len(response.Response.DecryptKey) < int(limit) {
			break
		}
...
func (me *KmsService) DescribeKmsDescribeWhiteBoxDeviceFingerprintsByFilter(ctx context.Context, param map[string]interface{}) (describeWhiteBoxDeviceFingerprints []*kms.DeviceFingerprint, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = kms.NewDescribeWhiteBoxDeviceFingerprintsRequest()
	)
	...
	for {
		...
		if response == nil || len(response.Response.DeviceFingerprints) < 1 {
			break
		}
		describeWhiteBoxDeviceFingerprints = append(describeWhiteBoxDeviceFingerprints, response.Response.DeviceFingerprints...)
		if len(response.Response.DeviceFingerprints) < int(limit) {
			break
		}
...
func (me *KmsService) DescribeKmsDescribeWhiteBoxKeyDetailsByFilter(ctx context.Context, param map[string]interface{}) (describeWhiteBoxKeyDetails []*kms.DescribeWhiteBoxKeyDetailsResponseParams, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = kms.NewDescribeWhiteBoxKeyDetailsRequest()
	)
	...
	for {
		...
		describeWhiteBoxKeyDetails = append(describeWhiteBoxKeyDetails, response.Response.TotalCount...)
...
func (me *KmsService) DescribeKmsGetParametersForImportByFilter(ctx context.Context, param map[string]interface{}) (getParametersForImport []*kms.GetParametersForImportResponseParams, errRet error) {
	...
	for {
		...
		if response == nil || len(response.Response.KeyId) < 1 {
			break
		}
		getParametersForImport = append(getParametersForImport, response.Response.KeyId...)
		if len(response.Response.KeyId) < int(limit) {
			break
		}
...
func (me *KmsService) DescribeKmsGetPublicKeyByFilter(ctx context.Context, param map[string]interface{}) (getPublicKey []*kms.GetPublicKeyResponseParams, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = kms.NewGetPublicKeyRequest()
	)
	...
	for {
		...
		getPublicKey = append(getPublicKey, response.Response.KeyId...)
		if len(response.Response.KeyId) < int(limit) {
			break
		}
...
func (me *KmsService) DescribeKmsListAlgorithmsByFilter(ctx context.Context, param map[string]interface{}) (listAlgorithms []*kms.AlgorithmInfo, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = kms.NewListAlgorithmsRequest()
	)
	...
	for {
		...
		if response == nil || len(response.Response.SymmetricAlgorithms) < 1 {
			break
		}
		listAlgorithms = append(listAlgorithms, response.Response.SymmetricAlgorithms...)
		if len(response.Response.SymmetricAlgorithms) < int(limit) {
			break
		}
...
func (me *KmsService) DescribeKmsListKeysByFilter(ctx context.Context, param map[string]interface{}) (listKeys []*kms.ListKeysResponseParams, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = kms.NewListKeysRequest()
	)
	...
	for {
		...
		listKeys = append(listKeys, response.Response.TotalCount...)
		if len(response.Response.TotalCount) < int(limit) {
			break
		}
...
func (me *KmsService) DescribeKmsKeyById(ctx context.Context, keyId string) (key *kms.KeyMetadata, errRet error) {
	logId := getLogId(ctx)
	...
	key = response.Response.KeyMetadata[0]
	return
}

func (me *KmsService) DeleteKmsKeyById(ctx context.Context, keyId string) (errRet error) {
	logId := getLogId(ctx)
