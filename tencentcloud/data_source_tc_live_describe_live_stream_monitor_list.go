/*
Use this data source to query detailed information of live describe_live_stream_monitor_list
...
func dataSourceTencentCloudLiveDescribeLiveStreamMonitorList() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudLiveDescribeLiveStreamMonitorListRead,
		Schema: map[string]*schema.Schema{
			"index": {
				Required:    true,
				Type:        schema.TypeInt,
				Description: "The starting offset when querying a list.",
			},

			"total_num": {
				Computed:    true,
				Type:        schema.TypeInt,
				Description: "The number of live stream monitoring tasks under the account.Note: This field may return null, indicating that no valid value is available.",
			},
...
func dataSourceTencentCloudLiveDescribeLiveStreamMonitorListRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_live_describe_live_stream_monitor_list.read")()
	...
	if v, _ := d.GetOk("index"); v != nil {
		paramMap["Index"] = helper.IntUint64(v.(int))
	}

	service := LiveService{client: meta.(*TencentCloudClient).apiV3Conn}

	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeLiveDescribeLiveStreamMonitorListByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		totalNum = result
	...
	if liveStreamMonitors != nil {
		for _, liveStreamMonitorInfo := range liveStreamMonitors {
			...
			if liveStreamMonitorInfo.InputList != nil {
				...
				liveStreamMonitorInfoMap["input_list"] = []interface{}{inputListList}
			}
