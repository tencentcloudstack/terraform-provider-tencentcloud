/*
Provides a resource to create a mps edit_media

Example Usage

```hcl
resource "tencentcloud_mps_edit_media" "edit_media" {
  file_infos {
		input_info {
			type = ""
			cos_input_info {
...
mps edit_media can be imported using the id, e.g.

```
terraform import tencentcloud_mps_edit_media.edit_media edit_media_id
```
*/
...
func resourceTencentCloudMpsEditMedia() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudMpsEditMediaCreate,
		Read:   resourceTencentCloudMpsEditMediaRead,
		Delete: resourceTencentCloudMpsEditMediaDelete,
...
func resourceTencentCloudMpsEditMediaCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_edit_media.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s operate mps editMedia failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudMpsEditMediaRead(d, meta)
}

func resourceTencentCloudMpsEditMediaRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_edit_media.read")()
	defer inconsistentCheck(d, meta)()

	return nil
}

func resourceTencentCloudMpsEditMediaDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_edit_media.delete")()
	defer inconsistentCheck(d, meta)()
