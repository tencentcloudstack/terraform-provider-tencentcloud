...
func dataSourceTencentCloudVpcNatDcRoute() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudVpcNatDcRouteRead,
...
func dataSourceTencentCloudVpcNatDcRouteRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_vpc_nat_dc_route.read")()
	...
	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeVpcNatDcRouteByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
	...
	if natDirectConnectGatewayRouteSet != nil {
		for _, natDirectConnectGatewayRoute := range natDirectConnectGatewayRouteSet {
			...
			ids = append(ids, *natDirectConnectGatewayRoute.NatGatewayId)
