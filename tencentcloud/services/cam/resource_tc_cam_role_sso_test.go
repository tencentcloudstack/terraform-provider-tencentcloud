...
func init() {
	...
	resource.AddTestSweepers("tencentcloud_cam_role_sso", &resource.Sweeper{
		...
		F: func(r string) error {
			...
			log.Printf("[%s] success, request %s, response %s, logId %s", dReq.GetAction(), dReq.ToJsonString(), dRes.ToJsonString(), ctx.Value(tccommon.LogIdKey))
...
func testAccCheckCamRoleSSODestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		response, err := tcacctest.AccProvider.Meta().(tccommon.ProviderMeta).GetAPIV3Conn().UseCamClient().DescribeOIDCConfig(request)
		if err != nil {
