...
func NewGaapService(client *connectivity.TencentCloudClient) GaapService {
	return GaapService{client: client}
}
...
func (me *GaapService) CreateProxy(
	...
	if err := resource.Retry(2*tccommon.WriteRetryTimeout, func() *resource.RetryError {
	...
	for count == 50 {
		...
		if err := resource.Retry(tccommon.ReadRetryTimeout, func() *resource.RetryError {
			...
			if err != nil {
				...
				log.Printf("[CRITAL]%s api[%s] fail, request body [%s], reason[%v]",
					logId, request.GetAction(), request.ToJsonString(), err)
				return tccommon.RetryError(err, GAAPInternalError)
			}
...
func (me *GaapService) DeleteLayer7Listener(ctx context.Context, id, proxyId, protocol string) error {
	...
	case "HTTP":
		...
		// don't set proxy id it may cause tccommon.InternalError
