	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"snat_enable": {
				Type:        schema.TypeBool,
				Optional:    true,
				Description: "Whether to enable SNAT (source IP replacement). True: enable, False: disable. Default is disabled. Note: When SnatEnable is enabled, it will replace the client source IP, and the transparent client source IP option will be disabled, and vice versa.",
			},
		},
	}
}
