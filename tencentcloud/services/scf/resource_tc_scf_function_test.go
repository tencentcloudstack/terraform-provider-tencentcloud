	...
	resource.AddTestSweepers("tencentcloud_scf", &resource.Sweeper{
		...
		F: func(r string) error {
			...
			for _, ns := range nss {
				...
				tccommon.ProcessScanCloudResources(client, resources, nonKeepResources, "scf", "")

				for _, fun := range funs {
					createTime := tccommon.StringToTime(*fun.AddTime)
					now := time.Now()
					interval := now.Sub(createTime).Minutes()
					if strings.HasPrefix(*fun.FunctionName, tcacctest.KeepResource) || strings.HasPrefix(*fun.FunctionName, tcacctest.DefaultResource) {
						continue
					}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("tencentcloud_scf_function.foo", "vpc_id", ""),
					resource.TestCheckResourceAttr("tencentcloud_scf_function.foo", "subnet_id", ""),
					resource.TestCheckResourceAttr("tencentcloud_scf_function.foo", "namespace", tcacctest.DefaultScfNamespace),
					resource.TestCheckResourceAttr("tencentcloud_scf_function.foo", "role", ""),
					resource.TestCheckResourceAttrSet("tencentcloud_scf_function.foo", "cls_logset_id"),
					resource.TestCheckResourceAttrSet("tencentcloud_scf_function.foo", "cls_topic_id"),
					resource.TestCheckResourceAttr("tencentcloud_scf_function.foo", "l5_enable", "false"),
...
var testAccScfFunctionBasicUpdate = fmt.Sprintf(tcacctest.DefaultVpcVariable+`
resource "tencentcloud_scf_function" "foo" {
  name      = "%s"
...
}`, tcacctest.DefaultSCFCosBucket, codeSource)
}
...
resource "tencentcloud_scf_function" "foo" {
  ...
  cos_bucket_region = "ap-guangzhou"
}`, tcacctest.DefaultSCFCosBucket, codeSource, codeSource, scfFunctionRandomName())
}
...
`, tcacctest.DefaultSCFCosBucket, "%s", "%s")
...
var testAccScfFunctionCfsConfigs = fmt.Sprintf(tcacctest.DefaultVpcVariable+tcacctest.DefaultFileSystem+`
...
`, "%s", "%s")
var testAccScfFunctionCfsConfigsUpdate = fmt.Sprintf(tcacctest.DefaultVpcVariable+tcacctest.DefaultFileSystem+`
