...
// NewBdrcRunCopyPairTasks returns action.Action, which already performs the
...
var _ action.ActionWithConfigure = &BdrcRunCopyPairTasks{}

// NewBdrcRunCopyPairTasks is the factory referenced by
// tencentcloud/framework/registry.go to register this action.
func NewBdrcRunCopyPairTasks() action.Action {
	return &BdrcRunCopyPairTasks{}
}

// BdrcRunCopyPairTasks implements action.Action for
...
func (a *BdrcRunCopyPairTasks) Metadata(_ context.Context, _ action.MetadataRequest, resp *action.MetadataResponse) {
	resp.TypeName = "tencentcloud_bdrc_run_copy_pair_tasks"
}
...
func (a *BdrcRunCopyPairTasks) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
	var data BdrcRunCopyPairTasksModel
