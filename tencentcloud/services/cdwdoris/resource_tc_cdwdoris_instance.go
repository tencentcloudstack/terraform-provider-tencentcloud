	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"is_ssc": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "Whether it is storage-compute separation. Default is false.",
			},
			"security_group_ids": {
				Type:        schema.TypeList,
				Optional:    true,
...
func resourceTencentCloudCdwdorisInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Create cdwdoris instance failed, Response is nil"))
		}
	...
	err = resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Describe cdwdoris instance state failed, Response is nil"))
		}

		if result.Response.InstanceState == nil {
			return resource.NonRetryableError(fmt.Errorf("Describe cdwdoris instance state failed, InstanceState is nil"))
		}
...
func resourceTencentCloudCdwdorisInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if respData == nil {
		log.Printf("[WARN]%s resource `tencentcloud_cdwdoris_instance` [%s] not found, please check if it has been deleted.", logId, d.Id())
		d.SetId("")
		return nil
	}
	...
	var workloadGroupStatus string
	if respData1 != nil {
		if respData1.Status != nil {
			workloadGroupStatus = *respData1.Status
		}
	}

	_ = d.Set("workload_group_status", workloadGroupStatus)

	if strings.HasPrefix(instanceId, DISABLE_SSC_PREFIX) {
		_ = d.Set("is_ssc", false)
	}

	if strings.HasPrefix(instanceId, ENABLE_SSC_PREFIX) {
		_ = d.Set("is_ssc", true)
	}
...
func resourceTencentCloudCdwdorisInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if chargeType == CHARGE_TYPE_POSTPAID_BY_HOUR {
		err = resource.Retry(d.Timeout(schema.TimeoutDelete), func() *resource.RetryError {
			...
			if result == nil || result.Response == nil {
				return resource.NonRetryableError(fmt.Errorf("Describe cdwdoris instances failed, Response is nil"))
			}
