...
func ResourceTencentCloudConfigUpdateConfigDeliver() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudConfigUpdateConfigDeliverCreate,
		Read:   resourceTencentCloudConfigUpdateConfigDeliverRead,
		Update: resourceTencentCloudConfigUpdateConfigDeliverUpdate,
		Delete: resourceTencentCloudConfigUpdateConfigDeliverDelete,
...
func resourceTencentCloudConfigUpdateConfigDeliverRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_config_update_config_deliver.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[WARN]%s resource `tencentcloud_config_update_config_deliver` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		d.SetId("")
		return nil
	}
...
func resourceTencentCloudConfigUpdateConfigDeliverUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_config_update_config_deliver.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s update config update_config_deliver failed, reason:%+v", logId, reqErr)
		return reqErr
	}

	return resourceTencentCloudConfigUpdateConfigDeliverRead(d, meta)
}

func resourceTencentCloudConfigUpdateConfigDeliverDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_config_update_config_deliver.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
