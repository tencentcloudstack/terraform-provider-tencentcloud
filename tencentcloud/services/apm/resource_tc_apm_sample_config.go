	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"sample_config_status": {
				...
				Description:  "Sample config status. 0: disabled; 1: enabled.",
			},
...
func resourceTencentCloudApmSampleConfigCreate(d *schema.ResourceData, meta interface{}) error {
	...
	reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseApmV20210622Client().CreateApmSampleConfigWithContext(ctx, request)
		if e != nil {
		...
		if result == nil || result.Response == nil || result.Response.ApmSampleConfig == nil {
			return resource.NonRetryableError(fmt.Errorf("Create apm sample config failed, Response is nil."))
		}
	...
	// set status close
	if v, ok := d.GetOkExists("sample_config_status"); ok {
		if v.(int) == 0 {
			...
			reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
				result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseApmV20210622Client().ModifyApmSampleConfigWithContext(ctx, request)
				if e != nil {
					return tccommon.RetryError(e)
				} else {
					log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudApmSampleConfigRead(d *schema.ResourceData, meta interface{}) error {
	...
	if respData.Tags != nil && len(respData.Tags) > 0 {
		...
		for _, item := range respData.Tags {
			tagsMap := map[string]interface{}{}
			if item.Key != nil {
				tagsMap["key"] = item.Key
			}

			if item.Value != nil {
				tagsMap["value"] = item.Value
			}
...
func resourceTencentCloudApmSampleConfigUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	needChange := false
	mutableArgs := []string{"sample_rate", "service_name", "operation_name", "tags", "operation_type"}
...
func resourceTencentCloudApmSampleConfigDelete(d *schema.ResourceData, meta interface{}) error {
	...
	request.SampleName = &sampleName
	reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseApmV20210622Client().DeleteApmSampleConfigWithContext(ctx, request)
		if e != nil {
			return tccommon.RetryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
