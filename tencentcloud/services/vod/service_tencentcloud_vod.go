...
// CreateVodAigcApiToken creates an AIGC API Token for the specified VOD sub application.
// It returns the token string generated by the cloud side. The token value is sensitive
// and MUST NOT be printed in logs.
func (me *VodService) CreateVodAigcApiToken(ctx context.Context, subAppId uint64) (apiToken string, errRet error) {
	logId := tccommon.GetLogId(ctx)
	request := vod.NewCreateAigcApiTokenRequest()
	request.SubAppId = helper.Uint64(subAppId)

	var requestId string
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		ratelimit.Check(request.GetAction())
		response, e := me.client.UseVodClient().CreateAigcApiToken(request)
		if e != nil {
			return tccommon.RetryError(e)
		}
		if response == nil || response.Response == nil || response.Response.ApiToken == nil {
	...
	log.Printf("[DEBUG]%s api[%s] success, sub_app_id [%d], api_token [***], request_id [%s]\n",
		logId, request.GetAction(), subAppId, requestId)
	return
}

// DescribeVodAigcApiTokens returns all AIGC API Tokens under the specified sub application.
// The cloud API has ~30s data-sync delay after Create/Delete, so callers that need
// consistency should wrap this with resource.Retry.
func (me *VodService) DescribeVodAigcApiTokens(ctx context.Context, subAppId uint64) (apiTokens []string, errRet error) {
	logId := tccommon.GetLogId(ctx)
	request := vod.NewDescribeAigcApiTokensRequest()
	request.SubAppId = helper.Uint64(subAppId)

	var requestId string
	err := resource.Retry(tccommon.ReadRetryTimeout, func() *resource.RetryError {
		ratelimit.Check(request.GetAction())
		response, e := me.client.UseVodClient().DescribeAigcApiTokens(request)
		if e != nil {
			return tccommon.RetryError(e)
		}
		...
		for _, t := range response.Response.ApiTokens {
			if t != nil {
	...
	log.Printf("[DEBUG]%s api[%s] success, sub_app_id [%d], token_count [%d], request_id [%s]\n",
		logId, request.GetAction(), subAppId, len(apiTokens), requestId)
	return
}

// DescribeVodAigcApiTokenById returns whether the given apiToken currently exists
// under the specified sub application.
func (me *VodService) DescribeVodAigcApiTokenById(ctx context.Context, subAppId uint64, apiToken string) (exists bool, errRet error) {
	tokens, err := me.DescribeVodAigcApiTokens(ctx, subAppId)
	if err != nil {
		errRet = err
		return
	}
...
// DeleteVodAigcApiToken deletes the given AIGC API Token. Errors that indicate the
// resource is already gone (ResourceNotFound.*) are treated as success for idempotency.
func (me *VodService) DeleteVodAigcApiToken(ctx context.Context, subAppId uint64, apiToken string) (errRet error) {
	logId := tccommon.GetLogId(ctx)
	request := vod.NewDeleteAigcApiTokenRequest()
	request.SubAppId = helper.Uint64(subAppId)
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		ratelimit.Check(request.GetAction())
		response, e := me.client.UseVodClient().DeleteAigcApiToken(request)
		if e != nil {
