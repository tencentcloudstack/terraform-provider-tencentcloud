...
func (me *VpcService) DescribeVpcPrivateNatGatewayTranslationAclRuleById(ctx context.Context, natGatewayId, translationDirection, translationType, translationIp, aclRuleId string) (ret *vpc.TranslationAclRule, errRet error) {
	logId := tccommon.GetLogId(ctx)
	...
	var (
		offset  uint64 = 0
		limit   uint64 = 100
		ruleSet []*vpc.TranslationAclRule
	)
	...
	for {
		...
		ruleSet = append(ruleSet, response.Response.TranslationAclRuleSet...)
	...
	for _, item := range ruleSet {
		if item != nil && item.AclRuleId != nil && helper.UInt64ToStr(*item.AclRuleId) == aclRuleId {
			ret = item
			return
		}
	}
