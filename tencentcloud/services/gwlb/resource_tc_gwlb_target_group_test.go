...
func TestAccTencentCloudGwlbTargetGroupResource_defaults(t *testing.T) {
	t.Parallel()
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			tcacctest.AccPreCheck(t)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccGwlbTargetGroupDefaults,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("tencentcloud_gwlb_target_group.gwlb_target_group", "id"),
					resource.TestCheckResourceAttr("tencentcloud_gwlb_target_group.gwlb_target_group", "health_check.0.timeout", "2"),
					resource.TestCheckResourceAttr("tencentcloud_gwlb_target_group.gwlb_target_group", "health_check.0.interval_time", "5"),
					resource.TestCheckResourceAttr("tencentcloud_gwlb_target_group.gwlb_target_group", "health_check.0.health_num", "3"),
					resource.TestCheckResourceAttr("tencentcloud_gwlb_target_group.gwlb_target_group", "health_check.0.un_health_num", "3"),
				),
			},
		},
	})
...
const testAccGwlbTargetGroupDefaults = `
resource "tencentcloud_vpc" "vpc" {
  name       = "vpc"
  cidr_block = "10.0.0.0/16"
}
