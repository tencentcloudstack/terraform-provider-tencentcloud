...
func ResourceTencentCloudTeoFunctionV2() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoFunctionV2Create,
		Read:   resourceTencentCloudTeoFunctionV2Read,
		Update: resourceTencentCloudTeoFunctionV2Update,
		Delete: resourceTencentCloudTeoFunctionV2Delete,
...
func resourceTencentCloudTeoFunctionV2Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_v2.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo function v2 failed, reason:%+v", logId, err)
		return err
	}
	...
	if _, err := (&resource.StateChangeConf{
		...
		Refresh:    resourceTeoFunctionV2CreateStateRefreshFunc_0_0(ctx, zoneId, functionId),
		Target:     []string{"true"},
	...
	return resourceTencentCloudTeoFunctionV2Read(d, meta)
}

func resourceTencentCloudTeoFunctionV2Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_v2.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	respData, err := service.DescribeTeoFunctionV2ById(ctx, zoneId, functionId)
	if err != nil {
		return err
	}
	...
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_function_v2` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoFunctionV2Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_v2.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo function v2 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoFunctionV2Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_v2.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo function v2 failed, reason:%+v", logId, err)
		return err
	}
...
func resourceTeoFunctionV2CreateStateRefreshFunc_0_0(ctx context.Context, zoneId string, functionId string) resource.StateRefreshFunc {
