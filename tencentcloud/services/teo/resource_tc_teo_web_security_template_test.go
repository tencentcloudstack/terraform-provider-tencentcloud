		...
		Steps: []resource.TestStep{
			{
				...
				Check:  resource.ComposeTestCheckFunc(resource.TestCheckResourceAttrSet("tencentcloud_teo_web_security_template.example", "id")),
			},
			{
				Config: testAccTeoWebSecurityTemplateUpdate,
				Check:  resource.ComposeTestCheckFunc(resource.TestCheckResourceAttrSet("tencentcloud_teo_web_security_template.example", "id")),
			},
			{
				ResourceName:      "tencentcloud_teo_web_security_template.example",
				ImportState:       true,
...
const testAccTeoWebSecurityTemplateUpdate = `
resource "tencentcloud_teo_web_security_template" "example" {
  zone_id       = "zone-3fkff38fyw8s"
  template_name = "example"
  security_policy {
    exception_rules {
      ...
      rules {
        name                               = "test1"
        condition                          = "$${http.request.host} in ['2.1.1.1']"
        skip_scope                         = "WebSecurityModules"
        skip_option                        = "SkipOnAllRequestFields"
        web_security_modules_for_exception = ["websec-mod-managed-rules"]
        enabled                            = "on"
