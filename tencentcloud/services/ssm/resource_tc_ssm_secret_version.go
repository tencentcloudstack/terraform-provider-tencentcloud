	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"version_id": {
				...
				ForceNew:    true,
				Required:    true,
				Description: "Specifies the version ID for the newly added version. It can be up to 64 bytes in length and must consist of a combination of letters, numbers, and the characters `-`, `_`, or `.`, starting with a letter or a number.",
			},
			"secret_binary": {
				Type:         schema.TypeString,
				Optional:     true,
				ExactlyOneOf: []string{"secret_string"},
				Description:  "Binary credential information, encoded using Base64. You must set exactly one of SecretBinary or SecretString.",
			},
			"secret_string": {
				Type:         schema.TypeString,
				Optional:     true,
				ExactlyOneOf: []string{"secret_binary"},
				Description:  "Text-based credential information in plaintext (Base64 encoding is not required). You must set exactly one of SecretBinary or SecretString.",
			},
		},
	}
}
