...
func (me *VpcService) DescribeSecurityGroups(ctx context.Context, sgId, sgName *string, projectId *int, tags map[string]string) (sgs []*vpc.SecurityGroup, err error) {
	...
	request.Limit = helper.String(strconv.Itoa(svccvm.DESCRIBE_SECURITY_GROUP_LIMIT))

	offset := 0
	count := svccvm.DESCRIBE_SECURITY_GROUP_LIMIT
	// run loop at least once
	for count == svccvm.DESCRIBE_SECURITY_GROUP_LIMIT {
