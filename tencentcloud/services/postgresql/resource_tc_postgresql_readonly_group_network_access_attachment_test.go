...
func testAccCheckReadonlyGroupNetworkAccessAttachmentDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		object, err := service.DescribePostgresqlReadonlyGroupSpecificNetInfoById(ctx, rs.Primary.ID)
...
const testAccPostgresqlInstanceForROGroupNetworkAccess = `
  variable "default_az" {
  ...
  locals {
	...
	pgsql_id = tencentcloud_postgresql_instance.test.id
  }
