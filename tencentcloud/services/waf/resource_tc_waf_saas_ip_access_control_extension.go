...
func resourceTencentCloudWafSaasIpAccessControlReadPostHandleResponse0(ctx context.Context, resp *wafv20180125.DescribeIpAccessControlResponseParams) error {
...
func resourceTencentCloudWafSaasIpAccessControlUpdatePreRequest0(ctx context.Context, req *wafv20180125.ModifyIpAccessControlRequest) *resource.RetryError {
	d := tccommon.ResourceDataFromContext(ctx)
...
func resourceTencentCloudWafSaasIpAccessControlDeletePreRequest0(ctx context.Context, req *wafv20180125.DeleteIpAccessControlV2Request) *resource.RetryError {
	d := tccommon.ResourceDataFromContext(ctx)
	idSplit := strings.Split(d.Id(), tccommon.FILED_SP)
	if len(idSplit) != 3 {
		return resource.NonRetryableError(fmt.Errorf("id is broken,%s", d.Id()))
