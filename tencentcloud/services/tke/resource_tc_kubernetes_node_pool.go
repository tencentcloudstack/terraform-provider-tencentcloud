		...
		Schema: map[string]*schema.Schema{
			...
			"retry_policy": {
				...
				ValidateFunc: tccommon.ValidateAllowedStringValue([]string{svcas.SCALING_GROUP_RETRY_POLICY_IMMEDIATE_RETRY,
					svcas.SCALING_GROUP_RETRY_POLICY_INCREMENTAL_INTERVALS, svcas.SCALING_GROUP_RETRY_POLICY_NO_RETRY}),
			},
			"vpc_id": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"multi_zone_subnet_policy": {
				...
				ValidateFunc: tccommon.ValidateAllowedStringValue([]string{svcas.MultiZoneSubnetPolicyPriority,
					svcas.MultiZoneSubnetPolicyEquality}),
...
func resourceKubernetesNodePoolCreate(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId(clusterId + tccommon.FILED_SP + nodePoolId)
