...
func resourceTencentCloudKubernetesNodePoolCreate(d *schema.ResourceData, meta interface{}) error {
	...
	var (
		...
		response = tke.NewCreateClusterNodePoolResponse()
	)
	...
	if instanceAdvancedSettingsMap, ok := helper.InterfacesHeadMap(d, "node_config"); ok {
		...
		if gPUArgsMap, ok := helper.ConvertInterfacesHeadToMap(instanceAdvancedSettingsMap["gpu_args"]); ok {
			...
			if driverMap, ok := helper.ConvertInterfacesHeadToMap(gPUArgsMap["driver"]); ok {
				driverVersion := tke.DriverVersion{}
				if v, ok := driverMap["version"]; ok {
					driverVersion.Version = helper.String(v.(string))
				}
				if v, ok := driverMap["name"]; ok {
					driverVersion.Name = helper.String(v.(string))
				}
				gPUArgs.Driver = &driverVersion
			}
			if cUDAMap, ok := helper.ConvertInterfacesHeadToMap(gPUArgsMap["cuda"]); ok {
				driverVersion := tke.DriverVersion{}
				if v, ok := cUDAMap["version"]; ok {
					driverVersion.Version = helper.String(v.(string))
				}
				if v, ok := cUDAMap["name"]; ok {
					driverVersion.Name = helper.String(v.(string))
				}
				gPUArgs.CUDA = &driverVersion
			}
			if cUDNNMap, ok := helper.ConvertInterfacesHeadToMap(gPUArgsMap["cudnn"]); ok {
				cUDNN := tke.CUDNN{}
				if v, ok := cUDNNMap["version"]; ok {
					cUDNN.Version = helper.String(v.(string))
				}
				if v, ok := cUDNNMap["name"]; ok {
					cUDNN.Name = helper.String(v.(string))
				}
				if v, ok := cUDNNMap["doc_name"]; ok {
					cUDNN.DocName = helper.String(v.(string))
				}
				if v, ok := cUDNNMap["dev_name"]; ok {
					cUDNN.DevName = helper.String(v.(string))
				}
				gPUArgs.CUDNN = &cUDNN
			}
			if customDriverMap, ok := helper.ConvertInterfacesHeadToMap(gPUArgsMap["custom_driver"]); ok {
				customDriver := tke.CustomDriver{}
				if v, ok := customDriverMap["address"]; ok {
					customDriver.Address = helper.String(v.(string))
				}
				gPUArgs.CustomDriver = &customDriver
			}
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTkeClient().CreateClusterNodePoolWithContext(ctx, request)
		if e != nil {
...
func resourceTencentCloudKubernetesNodePoolRead(d *schema.ResourceData, meta interface{}) error {
	...
	respData1, err := service.DescribeKubernetesNodePoolById1(ctx, clusterId, nodePoolId)
	if err != nil {
		return err
	}
	...
	if err != nil {
		log.Printf("[CRITAL]%s read kubernetes node pool failed, reason:%+v", logId, err)
		return err
	}
...
func resourceTencentCloudKubernetesNodePoolUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if needChange {
		...
		err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
			result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTkeClient().ModifyClusterNodePoolWithContext(ctx, request)
			if e != nil {
				return tccommon.RetryError(e)
			} else {
				log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudKubernetesNodePoolDelete(d *schema.ResourceData, meta interface{}) error {
	...
	var (
		...
		response = tke.NewDeleteClusterNodePoolResponse()
	)
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTkeClient().DeleteClusterNodePoolWithContext(ctx, request)
		if e != nil {
