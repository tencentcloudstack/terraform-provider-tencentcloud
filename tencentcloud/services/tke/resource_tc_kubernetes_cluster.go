...
func TkeExistCvmCreateInfo() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"node_role": {
			...
			ForceNew:     true,
			Optional:     true,
			ValidateFunc: tccommon.ValidateAllowedStringValue([]string{TKE_ROLE_WORKER, TKE_ROLE_MASTER_ETCD}),
	...
	if d.HasChange("project_id") || d.HasChange("cluster_name") || d.HasChange("cluster_desc") || d.HasChange("cluster_level") || d.HasChange("auto_upgrade_cluster_level") {
		...
		err = resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
			...
			if err != nil {
				...
				return tccommon.RetryError(err, tke.INTERNALERROR_UNEXPECTEDINTERNAL, tke.RESOURCEUNAVAILABLE)
			}
			return nil
		})
