...
func testSweepEksClusters(region string) error {
	client, err := tcacctest.SharedClientForRegion(region)
	logId := tccommon.GetLogId(tccommon.ContextNil)
	ctx := context.WithValue(context.TODO(), tccommon.LogIdKey, logId)
	if err != nil {
		return err
	}
	service := svctke.NewEksService(client.(tccommon.ProviderMeta).GetAPIV3Conn())
...
const testAccEksCluster = tcacctest.DefaultVpcVariable + `
...
const testAccEksClusterUpdate = tcacctest.DefaultVpcVariable + `
resource "tencentcloud_eks_cluster" "foo" {
