	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"meta_tags": {
				...
				Computed:    true,
				Optional:    true,
				Description: "Machine group metadata information list, stored as key-value pairs.",
			},
			"ostype": {
				Type:        schema.TypeInt,
				Computed:    true,
				Optional:    true,
				Description: "Operating system type. Valid values: 0 (Linux), 1 (Windows).",
			},
		},
	}
}
...
func resourceTencentCloudClsMachineGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("meta_tags"); ok {
		metaTags := v.(map[string]interface{})
		for key, value := range metaTags {
			k := key
			v := value.(string)
			request.MetaTags = append(request.MetaTags, &cls.MetaTagInfo{
				Key:   &k,
				Value: &v,
			})
		}
	...
	if v, ok := d.GetOkExists("ostype"); ok {
...
func resourceTencentCloudClsMachineGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	if machineGroup.MetaTags != nil {
		metaTags := make(map[string]string, len(machineGroup.MetaTags))
		for _, metaTag := range machineGroup.MetaTags {
			if metaTag.Key != nil && metaTag.Value != nil {
				metaTags[*metaTag.Key] = *metaTag.Value
			}
		}
		_ = d.Set("meta_tags", metaTags)
	}

	if machineGroup.OSType != nil {
		_ = d.Set("ostype", int(*machineGroup.OSType))
	}
