...
func resourceTencentCloudGa2GlobalAcceleratorAclRuleSetCreate(d *schema.ResourceData, meta interface{}) error {
	...
	// The API accepts at most 50 acl entries per request, so chunk the input
	// and issue one request per batch, then wait for each async task to finish.
	for _, chunk := range chunkSlice(entries, maxAclEntriesPerRequest) {
		...
		request.AclEntries = chunk

		var response = ga2v20250115.NewCreateGlobalAcceleratorAclRuleResponse()
		...
		if len(response.Response.GlobalAcceleratorAclRuleIds) == 0 {
			return fmt.Errorf("Create ga2 global_accelerator_acl_rule_set failed, GlobalAcceleratorAclRuleIds is nil or empty.")
		}

		if len(response.Response.GlobalAcceleratorAclRuleIds) != len(chunk) {
			return fmt.Errorf("Create ga2 global_accelerator_acl_rule_set failed, the count of GlobalAcceleratorAclRuleIds [%d] does not match the count of input AclEntries [%d].", len(response.Response.GlobalAcceleratorAclRuleIds), len(chunk))
		}
...
func resourceTencentCloudGa2GlobalAcceleratorAclRuleSetUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("acl_entries") {
		...
		// (b) batch-delete removed entries, chunked into batches of 50 rule ids.
		if len(toRemove) > 0 {
			...
			for _, chunk := range chunkSlice(ruleIds, maxAclEntriesPerRequest) {
				request := ga2v20250115.NewDeleteGlobalAcceleratorAclRuleRequest()
				request.GlobalAcceleratorId = helper.String(gaId)
				request.GlobalAcceleratorAclPolicyId = helper.String(policyId)
				request.GlobalAcceleratorAclRuleIds = chunk
		...
		// (a) batch-create new entries, chunked into batches of 50 entries.
		if len(toCreate) > 0 {
			...
			for _, chunk := range chunkSlice(aclEntries, maxAclEntriesPerRequest) {
				request := ga2v20250115.NewCreateGlobalAcceleratorAclRuleRequest()
				request.GlobalAcceleratorId = helper.String(gaId)
				request.GlobalAcceleratorAclPolicyId = helper.String(policyId)
				request.AclEntries = chunk
				...
				reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
					...
					if result.Response.GlobalAcceleratorAclRuleIds == nil || len(result.Response.GlobalAcceleratorAclRuleIds) != len(chunk) {
...
func resourceTencentCloudGa2GlobalAcceleratorAclRuleSetDelete(d *schema.ResourceData, meta interface{}) error {
	...
	// The API accepts at most 50 rule ids per request, so chunk and delete in
	// batches, waiting for each async task to finish.
	for _, chunk := range chunkSlice(ruleIds, maxAclEntriesPerRequest) {
		request := ga2v20250115.NewDeleteGlobalAcceleratorAclRuleRequest()
		request.GlobalAcceleratorId = helper.String(gaId)
		request.GlobalAcceleratorAclPolicyId = helper.String(policyId)
...
// maxAclEntriesPerRequest is the maximum number of acl entries (create) or rule
// ids (delete) accepted by the ga2 ACL APIs in a single request.
const maxAclEntriesPerRequest = 50

// chunkSlice splits items into contiguous sub-slices of at most size elements.
// The elements are returned in their original order.
