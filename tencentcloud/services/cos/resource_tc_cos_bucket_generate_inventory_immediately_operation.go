...
func resourceTencentCloudCosBucketGenerateInventoryImmediatelyOperationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	result, _, err := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTencentCosClient(bucket, cdcId).Bucket.GetInventory(ctx, inventoryId)
	if err != nil {
	...
	_, err = meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTencentCosClient(bucket, cdcId).Bucket.PostInventory(ctx, id, inventoryOpt)
	if err != nil {
		return err
	}
	...
	err = resource.Retry(10*tccommon.ReadRetryTimeout, func() *resource.RetryError {
		resp, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTencentCosClient(targetBucket, cdcId).Object.Head(ctx, objectId, nil)
