...
func resourceTencentCloudTsfClusterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, _ := d.GetOk("max_node_pod_num"); v != nil {
		request.MaxNodePodNum = helper.IntUint64(v.(int))
	}

	if v, _ := d.GetOk("max_cluster_service_num"); v != nil {
	...
	if _, ok := d.GetOk("cluster_remark_name"); ok {
		return fmt.Errorf("There is no need to remark name when creating a cluster")
	}
