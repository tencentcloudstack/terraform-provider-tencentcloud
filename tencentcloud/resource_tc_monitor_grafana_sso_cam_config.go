...
```
terraform import tencentcloud_monitor_grafana_sso_cam_config.grafana_sso_cam_config grafana_sso_cam_config_id
```
*/
...
func resourceTencentCloudMonitorGrafanaSsoCamConfigRead(d *schema.ResourceData, meta interface{}) error {
	...
	grafanaSsoCamConfigId := d.Id()

	grafanaSsoCamConfig, err := service.DescribeMonitorGrafanaSsoCamConfigById(ctx, instanceId)
	if err != nil {
		return err
	}
	...
	if grafanaSsoCamConfig.InstanceId != nil {
		_ = d.Set("instance_id", grafanaSsoCamConfig.InstanceId)
	}

	if grafanaSsoCamConfig.EnableSSOCamCheck != nil {
		_ = d.Set("enable_s_s_o_cam_check", grafanaSsoCamConfig.EnableSSOCamCheck)
	}
...
func resourceTencentCloudMonitorGrafanaSsoCamConfigUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	immutableArgs := []string{"instance_id", "enable_s_s_o_cam_check"}
	...
	if d.HasChange("enable_s_s_o_cam_check") {
		if v, ok := d.GetOkExists("enable_s_s_o_cam_check"); ok {
