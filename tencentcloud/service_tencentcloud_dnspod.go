...
func (me *DnspodService) DescribeDnspodCustom_lineById(ctx context.Context, domain string, name string) (custom_line *dnspod.CustomLineInfo, errRet error) {
	logId := getLogId(ctx)
	...
	if len(response.Response.CustomLineInfo) < 1 {
		return
	}

	custom_line = response.Response.CustomLineInfo[0]
	return
}

func (me *DnspodService) DeleteDnspodCustom_lineById(ctx context.Context, domain string, name string) (errRet error) {
	logId := getLogId(ctx)
