...
func TestAccTencentCloudMpsProcessMediaOperationResource_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(testAccMpsProcessMediaOperation, defaultRegion, defaultRegion),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("tencentcloud_mps_process_media_operation.operation", "input_info.0.cos_input_info.0.region", defaultRegion),
					...
					resource.TestCheckResourceAttr("tencentcloud_mps_process_media_operation.operation", "output_storage.0.cos_output_storage.0.region", defaultRegion),
				),
			},
		},
	})
...
const testAccMpsProcessMediaOperation = userInfoData + `
