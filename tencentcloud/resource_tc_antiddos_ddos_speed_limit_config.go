...
func resourceTencentCloudAntiddosDdosSpeedLimitConfigCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_ddos_speed_limit_config.create")()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseAntiddosClient().CreateDDoSSpeedLimitConfig(request)
		if e != nil {
			return retryError(e)
		} else {
...
func resourceTencentCloudAntiddosDdosSpeedLimitConfigRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_ddos_speed_limit_config.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudAntiddosDdosSpeedLimitConfigUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_ddos_speed_limit_config.update")()
	defer inconsistentCheck(d, meta)()
	...
	if d.HasChange("ddos_speed_limit_config") {
		...
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			result, e := meta.(*TencentCloudClient).apiV3Conn.UseAntiddosClient().ModifyDDoSSpeedLimitConfig(request)
			if e != nil {
				return retryError(e)
			} else {
				log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudAntiddosDdosSpeedLimitConfigDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_ddos_speed_limit_config.delete")()
	defer inconsistentCheck(d, meta)()
