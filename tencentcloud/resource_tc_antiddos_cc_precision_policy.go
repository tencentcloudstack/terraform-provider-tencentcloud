...
func resourceTencentCloudAntiddosCcPrecisionPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_cc_precision_policy.create")()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseAntiddosClient().CreateCCPrecisionPolicy(request)
		if e != nil {
			return retryError(e)
		} else {
	...
	for _, item := range ccPrecisionPolicys {
		...
		for _, policy := range item.PolicyList {
			...
			tmpPolicyStrings = append(tmpPolicyStrings, strings.Join(policyItems, FILED_SP))
		}
	...
	d.SetId(strings.Join([]string{instanceId, *ccPrecisionPolicy.PolicyId, ip, domain, protocol}, FILED_SP))
...
func resourceTencentCloudAntiddosCcPrecisionPolicyRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_cc_precision_policy.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudAntiddosCcPrecisionPolicyUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_cc_precision_policy.update")()
	defer inconsistentCheck(d, meta)()
	...
	if d.HasChange("policy_action") || d.HasChange("policy_list") {
		...
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			result, e := meta.(*TencentCloudClient).apiV3Conn.UseAntiddosClient().ModifyCCPrecisionPolicy(request)
			if e != nil {
				return retryError(e)
			} else {
				log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudAntiddosCcPrecisionPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_antiddos_cc_precision_policy.delete")()
	defer inconsistentCheck(d, meta)()
