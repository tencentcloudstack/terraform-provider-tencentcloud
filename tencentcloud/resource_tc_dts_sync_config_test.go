...
const testAccDtsSyncConfig_ccn = testAccDtsMigrateJob_vpc_config + `
resource "tencentcloud_ccn" "dst" {
	name                 = "tf_test_dts_cdb_2_ccn"
	description          = "tf dts test sync cdb to ccn"
	qos                  = "AG"
...
resource "tencentcloud_dts_sync_config" "sync_config" {
  ...
  job_name = "tf_test_sync_config_ccn"
  job_mode = "liteMode"
  run_mode = "Immediate"
  ...
  dst_info {
		region        = "ap-guangzhou"
		instance_id   = tencentcloud_ccn.dst.id
		user          = "root"
		password      = "cynos@123"
		db_name       = "tf_ci_test_new"
		vpc_id        = local.vpc_id
