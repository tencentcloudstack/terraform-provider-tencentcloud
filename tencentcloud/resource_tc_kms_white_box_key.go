...
```
terraform import tencentcloud_kms_white_box_key.white_box_key white_box_key_id
```
*/
...
func resourceTencentCloudKmsWhiteBoxKeyUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	var (
		...
		enableWhiteBoxKeyResponse = kms.NewEnableWhiteBoxKeyResponse()
	)
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseKmsClient().EnableWhiteBoxKey(request)
		if e != nil {
			return retryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
	...
	if err != nil {
		log.Printf("[CRITAL]%s update kms whiteBoxKey failed, reason:%+v", logId, err)
		return err
	}
