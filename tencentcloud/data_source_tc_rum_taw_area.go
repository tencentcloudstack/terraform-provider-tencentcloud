...
func dataSourceTencentCloudRumTawAreaRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_rum_taw_area.read")()
	...
	if v, ok := d.GetOk("area_ids"); ok {
		...
		for i := range areaIdsSet {
			...
			paramMap["AreaIds"] = append(paramMap["AreaIds"], helper.IntInt64(areaIds))
		}
	}
	...
	if v, ok := d.GetOk("area_statuses"); ok {
		...
		for i := range areaStatusesSet {
			...
			paramMap["AreaStatuses"] = append(paramMap["AreaStatuses"], helper.IntInt64(areaStatuses))
		}
	}
	...
	if areaSet != nil {
		for _, rumAreaInfo := range areaSet {
			...
			if rumAreaInfo.AreaRegionID != nil {
				rumAreaInfoMap["area_region_i_d"] = rumAreaInfo.AreaRegionID
			}
			...
			ids = append(ids, *rumAreaInfo.AreaId)
