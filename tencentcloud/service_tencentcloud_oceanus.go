...
func (me *OceanusService) DescribeOceanusCheckSavepointByFilter(ctx context.Context, param map[string]interface{}) (CheckSavepoint []*oceanus.CheckSavepointResponseParams, errRet error) {
	...
	for {
		...
		if response == nil || len(response.Response.SerialId) < 1 {
			break
		}
		CheckSavepoint = append(CheckSavepoint, response.Response.SerialId...)
		if len(response.Response.SerialId) < int(limit) {
			break
		}
...
func (me *OceanusService) DescribeOceanusClusterByFilter(ctx context.Context, param map[string]interface{}) (Cluster []*oceanus.Cluster, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = oceanus.NewDescribeClustersRequest()
	)
	...
	for {
		...
		Cluster = append(Cluster, response.Response.ClusterSet...)
		if len(response.Response.ClusterSet) < int(limit) {
			break
		}
...
func (me *OceanusService) DescribeOceanusLogByFilter(ctx context.Context, param map[string]interface{}) (log []*oceanus.DescribeJobSubmissionLogResponseParams, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = oceanus.NewDescribeJobSubmissionLogRequest()
	)
	...
	for {
		...
		if response == nil || len(response.Response.Cursor) < 1 {
			break
		}
		log = append(log, response.Response.Cursor...)
		if len(response.Response.Cursor) < int(limit) {
			break
		}
