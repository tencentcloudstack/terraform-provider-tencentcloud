...
func dataSourceTencentCloudMonitorAlarmPolicyRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_monitor_alarm_policy.read")()
	...
	if v, ok := d.GetOk("receiver_uids"); ok {
		...
		for i := range receiverUidsSet {
			...
			paramMap["ReceiverUids"] = append(paramMap["ReceiverUids"], helper.IntInt64(receiverUids))
		}
	}
	...
	if v, ok := d.GetOk("receiver_groups"); ok {
		...
		for i := range receiverGroupsSet {
			...
			paramMap["ReceiverGroups"] = append(paramMap["ReceiverGroups"], helper.IntInt64(receiverGroups))
		}
	}
	...
	if v, ok := d.GetOk("enable"); ok {
		...
		for i := range enableSet {
			...
			paramMap["Enable"] = append(paramMap["Enable"], helper.IntInt64(enable))
		}
	}

	if v, _ := d.GetOk("not_binding_notice_rule"); v != nil {
		paramMap["NotBindingNoticeRule"] = helper.IntInt64(v.(int))
	}

	if v, _ := d.GetOk("instance_group_id"); v != nil {
		paramMap["InstanceGroupId"] = helper.IntInt64(v.(int))
	}

	if v, _ := d.GetOk("need_correspondence"); v != nil {
	...
	if v, _ := d.GetOk("not_bind_all"); v != nil {
		paramMap["NotBindAll"] = helper.IntInt64(v.(int))
	}

	if v, _ := d.GetOk("not_instance_group"); v != nil {
	...
	if v, ok := d.GetOk("receiver_on_call_form_i_ds"); ok {
		...
		for _, alarmPolicy := range policies {
			...
			if alarmPolicy.Condition != nil {
				...
				if alarmPolicy.Condition.Rules != nil {
					...
					conditionMap["rules"] = []interface{}{rulesList}
				}
			...
			if alarmPolicy.EventCondition != nil {
				...
				if alarmPolicy.EventCondition.Rules != nil {
					...
					eventConditionMap["rules"] = []interface{}{rulesList}
				}
			...
			if alarmPolicy.Notices != nil {
				...
				for _, notices := range alarmPolicy.Notices {
					...
					if notices.UserNotices != nil {
						...
						for _, userNotices := range notices.UserNotices {
							...
							if userNotices.OnCallFormIDs != nil {
								userNoticesMap["on_call_form_i_ds"] = userNotices.OnCallFormIDs
							}

							userNoticesList = append(userNoticesList, userNoticesMap)
						}

						noticesMap["user_notices"] = []interface{}{userNoticesList}
					}
					...
					if notices.URLNotices != nil {
						...
						for _, uRLNotices := range notices.URLNotices {
							...
							if uRLNotices.URL != nil {
								uRLNoticesMap["u_r_l"] = uRLNotices.URL
							}
						...
						noticesMap["u_r_l_notices"] = []interface{}{uRLNoticesList}
					}
					...
					if notices.AMPConsumerId != nil {
						noticesMap["a_m_p_consumer_id"] = notices.AMPConsumerId
					}
					...
					if notices.CLSNotices != nil {
						...
						noticesMap["c_l_s_notices"] = []interface{}{cLSNoticesList}
					}
					...
					if notices.Tags != nil {
						...
						noticesMap["tags"] = []interface{}{tagsList}
					}

					noticesList = append(noticesList, noticesMap)
				}

				alarmPolicyMap["notices"] = []interface{}{noticesList}
			}
			...
			if alarmPolicy.TriggerTasks != nil {
				...
				alarmPolicyMap["trigger_tasks"] = []interface{}{triggerTasksList}
			}
			...
			if alarmPolicy.TagInstances != nil {
				...
				alarmPolicyMap["tag_instances"] = []interface{}{tagInstancesList}
			}
			...
			if alarmPolicy.Tags != nil {
				...
				alarmPolicyMap["tags"] = []interface{}{tagsList}
			}

			ids = append(ids, *alarmPolicy.AlarmId)
