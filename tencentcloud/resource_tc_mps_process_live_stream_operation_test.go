...
func TestAccTencentCloudMpsProcessLiveStreamOperationResource_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(testAccMpsProcessLiveStreamOperation, defaultRegion),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("tencentcloud_mps_process_live_stream_operation.operation", "output_storage.0.cos_output_storage.0.region", defaultRegion),
...
const testAccMpsProcessLiveStreamOperation = userInfoData + `
