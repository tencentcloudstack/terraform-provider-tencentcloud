...
func resourceTencentCloudRedisReplicaReadonlyRead(d *schema.ResourceData, meta interface{}) error {
	...
	ReplicaReadonly, err := service.DescribeRedisReplicaReadonlyById(ctx, instanceId)
	if err != nil {
		return err
	}

	if ReplicaReadonly == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `RedisReplicaReadonly` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}

	if ReplicaReadonly.InstanceId != nil {
		_ = d.Set("instance_id", ReplicaReadonly.InstanceId)
	}

	if ReplicaReadonly.ReadonlyPolicy != nil {
		_ = d.Set("readonly_policy", ReplicaReadonly.ReadonlyPolicy)
	}
...
func resourceTencentCloudRedisReplicaReadonlyUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	var (
		disableReplicaReadonlyRequest  = redis.NewDisableReplicaReadonlyRequest()
		disableReplicaReadonlyResponse = redis.NewDisableReplicaReadonlyResponse()
	)
