...
```hcl
data "tencentcloud_sqlserver_datasource_regions" "datasource_regions" {
  }
```
*/
...
func dataSourceTencentCloudSqlserverDatasourceRegions() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudSqlserverDatasourceRegionsRead,
...
func dataSourceTencentCloudSqlserverDatasourceRegionsRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_sqlserver_datasource_regions.read")()
	...
	var regionSet []*sqlserver.RegionInfo

	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeSqlserverDatasourceRegionsByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		regionSet = result
	...
	if regionSet != nil {
		for _, regionInfo := range regionSet {
			...
			ids = append(ids, *regionInfo.InstanceId)
