...
func dataSourceTencentCloudVpcVpnCustomerGatewayVendors() *schema.Resource {
...
func dataSourceTencentCloudVpcVpnCustomerGatewayVendorsRead(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeVpcVpnCustomerGatewayVendorsByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
	...
	if customerGatewayVendorSet != nil {
		for _, customerGatewayVendor := range customerGatewayVendorSet {
			...
			ids = append(ids, *customerGatewayVendor.CustomerGatewayVendor)
