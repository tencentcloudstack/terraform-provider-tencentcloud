...
data "tencentcloud_instance_types" "ins_type" {
  availability_zone = var.env_az != "" ? var.env_az :  "` + defaultCvmAZone + `"
  cpu_core_count    = 2
  exclude_sold_out  = true
}
...
data "tencentcloud_vpc_subnets" "sub" {
  vpc_id = data.tencentcloud_kubernetes_clusters.tke.list.0.vpc_id
}

locals {
  cluster_id = data.tencentcloud_kubernetes_clusters.tke.list.0.cluster_id
  cluster_vpc_id = data.tencentcloud_vpc_subnets.sub.instance_list.0.vpc_id
  cluster_subnet_id = data.tencentcloud_vpc_subnets.sub.instance_list.0.subnet_id
}
`
