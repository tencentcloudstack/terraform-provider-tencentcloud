/*
Use this data source to query detailed information of wedata datasouce__describe_data_source_without_info
...
func dataSourceTencentCloudWedataDatasouce_DescribeDataSourceWithoutInfo() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudWedataDatasouce_DescribeDataSourceWithoutInfoRead,
...
func dataSourceTencentCloudWedataDatasouce_DescribeDataSourceWithoutInfoRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_wedata_datasouce__describe_data_source_without_info.read")()
	...
	var data []*wedata.DataSourceInfo

	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeWedataDatasouce_DescribeDataSourceWithoutInfoByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		data = result
	...
	if data != nil {
		for _, dataSourceInfo := range data {
			...
			if dataSourceInfo.ID != nil {
				dataSourceInfoMap["i_d"] = dataSourceInfo.ID
			}
			...
			ids = append(ids, *dataSourceInfo.ID)
