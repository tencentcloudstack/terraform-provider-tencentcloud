/*
Use this data source to query detailed information of tsf describe_delivery_configs
...
func dataSourceTencentCloudTsfDescribeDeliveryConfigs() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudTsfDescribeDeliveryConfigsRead,
...
func dataSourceTencentCloudTsfDescribeDeliveryConfigsRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_tsf_describe_delivery_configs.read")()
	...
	var result []*tsf.DeliveryConfigBindGroups

	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		result, e := service.DescribeTsfDescribeDeliveryConfigsByFilter(ctx, paramMap)
		if e != nil {
			return retryError(e)
		}
		result = result
	...
	if result != nil {
		...
		if result.TotalCount != nil {
			deliveryConfigBindGroupsMap["total_count"] = result.TotalCount
		}
		...
		if result.Content != nil {
			...
			for _, content := range result.Content {
				...
				if content.Groups != nil {
					...
					contentMap["groups"] = []interface{}{groupsList}
				}
				...
				if content.KafkaInfos != nil {
					...
					contentMap["kafka_infos"] = []interface{}{kafkaInfosList}
				}

				contentList = append(contentList, contentMap)
			}

			deliveryConfigBindGroupsMap["content"] = []interface{}{contentList}
		}

		ids = append(ids, *result.IdsHash)
		_ = d.Set("result", deliveryConfigBindGroupsMap)
	}

	d.SetId(helper.DataResourceIdsHash(ids))
	output, ok := d.GetOk("result_output_file")
	if ok && output.(string) != "" {
		if e := writeToFile(output.(string), deliveryConfigBindGroupsMap); e != nil {
			return e
		}
	}
	return nil
