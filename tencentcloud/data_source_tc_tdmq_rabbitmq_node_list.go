...
func dataSourceTencentCloudTdmqRabbitmqNodeListRead(d *schema.ResourceData, meta interface{}) error {
	...
	var nodeList []*tdmq.RabbitMQPrivateNode
	...
	ids := make([]string, 0, len(nodeList))
	...
	if nodeList != nil {
		for _, rabbitMQPrivateNode := range nodeList {
			...
			if rabbitMQPrivateNode.CPUUsage != nil {
				rabbitMQPrivateNodeMap["c_p_u_usage"] = rabbitMQPrivateNode.CPUUsage
			}
			...
			ids = append(ids, *rabbitMQPrivateNode.InstanceId)
