...
func dataSourceTencentCloudAsScalingGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	for _, scalingGroup := range scalingGroups {
		...
		mapping := map[string]interface{}{
			...
			"subnet_ids":           helper.FlattenStringList(scalingGroup.SubnetIdSet),
			"zones":                helper.FlattenStringList(scalingGroup.ZoneSet),
			"default_cooldown":     scalingGroup.DefaultCooldown,
			"desired_capacity":     scalingGroup.DesiredCapacity,
			"load_balancer_ids":    helper.FlattenStringList(scalingGroup.LoadBalancerIdSet),
			"termination_policies": helper.FlattenStringList(scalingGroup.TerminationPolicySet),
