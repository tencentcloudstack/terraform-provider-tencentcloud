...
func dataSourceTencentCloudDtsCompareTasksRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloud_dts_compare_tasks.read")()
	...
	var items []*dts.CompareTaskItem
	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		results, e := dtsService.DescribeDtsCompareTasksByFilter(ctx, param)
		if e != nil {
			return retryError(e)
		}
		items = results
		...
		for _, item := range items {
			...
			if item.Config != nil {
				...
				if item.Config.ObjectItems != nil {
					...
					for _, objectItems := range item.Config.ObjectItems.ObjectItems {
						...
						if objectItems.Tables != nil {
							...
							for _, tables := range objectItems.Tables.Tables {
						...
						if objectItems.Views != nil {
							viewsList := []interface{}{}
							for _, views := range objectItems.Views.Views {
			...
			if item.CheckProcess != nil {
				...
				if item.CheckProcess.Step != nil {
					stepList := []interface{}{}
					for _, step := range item.CheckProcess.Step.Step {
						...
						if step.Errors != nil {
							...
							for _, errors := range step.Errors.Errors {
						...
						if step.Warnings != nil {
							...
							for _, warnings := range step.Warnings.Warnings {
			...
			if item.CompareProcess != nil {
				...
				if item.CompareProcess.Step != nil {
					stepList := []interface{}{}
					for _, step := range item.CompareProcess.Step.Step {
						stepMap := map[string]interface{}{}
						if step.StepNo != nil {
							stepMap["step_no"] = step.StepNo
						}
