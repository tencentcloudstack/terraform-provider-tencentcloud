/*
Provides a resource to create a mps process_media
...
mps process_media can be imported using the id, e.g.

```
terraform import tencentcloud_mps_process_media.process_media process_media_id
```
*/
...
func resourceTencentCloudMpsProcessMedia() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudMpsProcessMediaCreate,
		Read:   resourceTencentCloudMpsProcessMediaRead,
		Delete: resourceTencentCloudMpsProcessMediaDelete,
		...
		Schema: map[string]*schema.Schema{
			...
			"schedule_id": {
				...
				ForceNew:    true,
				Type:        schema.TypeInt,
				Description: "The scheme ID.Note 1: About `OutputStorage` and `OutputDir`&amp;amp;lt;li&amp;amp;gt;If an output storage and directory are specified for a subtask of the scheme, those output settings will be applied.&amp;amp;lt;/li&amp;amp;gt;&amp;amp;lt;li&amp;amp;gt;If an output storage and directory are not specified for the subtasks of a scheme, the output parameters passed in the `ProcessMedia` API will be applied.&amp;amp;lt;/li&amp;amp;gt;Note 2: If `TaskNotifyConfig` is specified, the specified settings will be used instead of the default callback settings of the scheme.Note 3: The trigger configured for a scheme is for automatically starting a scheme. It stops working when you manually call this API to start a scheme.",
			},
													...
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															"type": {
																Type:        schema.TypeString,
																Optional:    true,
																Description: "The TSC type. Valid values:&amp;lt;li&amp;gt;`TEHD-100`: TSC-100 (video TSC). &amp;lt;/li&amp;gt;&amp;lt;li&amp;gt;`TEHD-200`: TSC-200 (audio TSC). &amp;lt;/li&amp;gt;If this parameter is left blank, no modification will be made.Note: This field may return·null, indicating that no valid values can be obtained.",
															},
															"max_video_bitrate": {
																Type:        schema.TypeInt,
																Optional:    true,
													...
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															...
															"font_type": {
																Type:        schema.TypeString,
																Optional:    true,
																Description: "The font. Valid values:&amp;lt;li&amp;gt;`hei.ttf`: Heiti.&amp;lt;/li&amp;gt;&amp;lt;li&amp;gt;`song.ttf`: Songti.&amp;lt;/li&amp;gt;&amp;lt;li&amp;gt;`simkai.ttf`: Kaiti.&amp;lt;/li&amp;gt;&amp;lt;li&amp;gt;`arial.ttf`: Arial.&amp;lt;/li&amp;gt;The default is `hei.ttf`.Note: This field may return·null, indicating that no valid values can be obtained.",
															},
															"font_size": {
																Type:        schema.TypeString,
																Optional:    true,
															...
															"font_alpha": {
																...
																Optional:    true,
																Description: "The text transparency. Value range: 0-1.&amp;lt;li&amp;gt;`0`: Fully transparent.&amp;lt;/li&amp;gt;&amp;lt;li&amp;gt;`1`: Fully opaque.&amp;lt;/li&amp;gt;Default value: 1.Note: This field may return·null, indicating that no valid values can be obtained.",
															},
														},
													},
												},
													...
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															"type": {
																Type:        schema.TypeString,
																Optional:    true,
																Description: "The mode. Valid values:&amp;lt;li&amp;gt;`subtitle-stream`: Add a subtitle track.&amp;lt;/li&amp;gt;&amp;lt;li&amp;gt;`close-caption-708`: Embed CEA-708 subtitles in SEI frames.&amp;lt;/li&amp;gt;&amp;lt;li&amp;gt;`close-caption-608`: Embed CEA-608 subtitles in SEI frames.&amp;lt;/li&amp;gt;Note: This field may return·null, indicating that no valid values can be obtained.",
															},
															"subtitle": {
																Type:        schema.TypeList,
																MaxItems:    1,
			...
			"task_type": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "The task type.&amp;amp;lt;li&amp;amp;gt; `Online` (default): A task that is executed immediately.&amp;amp;lt;/li&amp;amp;gt;&amp;amp;lt;li&amp;amp;gt; `Offline`: A task that is executed when the system is idle (within three days by default).&amp;amp;lt;/li&amp;amp;gt;.",
			},
		},
	}
}

func resourceTencentCloudMpsProcessMediaCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_process_media.create")()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseMpsClient().ProcessMedia(request)
		if e != nil {
			return retryError(e)
		} else {
	...
	if err != nil {
		log.Printf("[CRITAL]%s operate mps processMedia failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudMpsProcessMediaRead(d, meta)
}

func resourceTencentCloudMpsProcessMediaRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_process_media.read")()
	defer inconsistentCheck(d, meta)()

	return nil
}

func resourceTencentCloudMpsProcessMediaDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloud_mps_process_media.delete")()
	defer inconsistentCheck(d, meta)()
