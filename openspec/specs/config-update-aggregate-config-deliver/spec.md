# config-update-aggregate-config-deliver Specification

## Purpose
Defines the `tencentcloud_config_update_aggregate_config_deliver` resource that manages the delivery settings (投递设置) of a Tencent Cloud Config account group (账号组) via the `DescribeAggregateConfigDeliver` and `UpdateAggregateConfigDeliver` cloud APIs.
## Requirements
