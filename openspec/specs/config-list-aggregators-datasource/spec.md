# config-list-aggregators-datasource Specification

## Purpose
Provides a Terraform data source `tencentcloud_config_list_aggregators` that queries the list of Config account groups (aggregators) via the Config `ListAggregators` API (config v20220802). Pagination is handled automatically in the service layer (`Limit=100`/`Offset=0` looping), and the data source exposes the account group total count and account group details list to Terraform.
