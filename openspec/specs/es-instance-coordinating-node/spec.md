...
This specification defines the requirements for supporting the `dedicatedCoordinating` (dedicated coordinating node) value for the `node_info_list[].type` field in the `tencentcloud_elasticsearch_instance` Terraform resource, covering schema, create, read, update flows, and documentation.
