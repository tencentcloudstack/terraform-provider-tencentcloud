# antiddos-ddos-block-records-datasource Specification

## Purpose
Provides a Terraform data source `tencentcloud_antiddos_ddos_block_records` that queries AntiDDoS DDoS block/unblock records via the `DescribeDDoSBlockRecords` API (antiddos v20250903). Supports querying by time range and optional filters, returns block records list and unblock quota info, and handles pagination automatically in the service layer.
## Requirements
