# apm-instance-log-span-id-key Specification

## Purpose
Defines that the `tencentcloud_apm_instance` resource SHALL support the `log_span_id_key` parameter, mapping to the `LogSpanIdKey` field of the TencentCloud APM `ModifyApmInstance` API, to complete the key-value CLS index configuration for APM business systems.
