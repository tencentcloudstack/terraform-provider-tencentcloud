# cvm-repair-task-control-operation Specification

## Purpose

定义 CVM 服务维修任务授权 operation 资源 `tencentcloud_cvm_repair_task_control_operation` 的行为规范，封装 `RepairTaskControl` 云 API，用于对待授权的维修任务执行立即/预约授权，并可选择启用弃盘迁移策略。
