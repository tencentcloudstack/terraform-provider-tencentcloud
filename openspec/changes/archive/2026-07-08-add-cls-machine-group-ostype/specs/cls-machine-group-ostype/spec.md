...
The `tencentcloud_cls_machine_group` resource SHALL support an `ostype` parameter of type `TypeInt` that specifies the operating system type of the machine group. The valid values are 0 (Linux) and 1 (Windows). This parameter is Optional and ForceNew.
...
#### Scenario: Change OSType triggers recreation
- **WHEN** a user changes `ostype` from 0 to 1 on an existing resource
- **THEN** Terraform SHALL destroy the existing machine group and create a new one with the updated OSType value