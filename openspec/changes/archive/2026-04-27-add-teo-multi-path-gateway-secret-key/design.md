...
2. **Create 方法设计**：采用 RESOURCE_KIND_CONFIG 的标准模式，Create 方法设置 zone_id 为资源 ID，然后直接调用 Update 方法。在 Update 方法中，通过 `d.IsNewResource()` 判断当前是否为 Create 操作：如果是 Create 操作，调用 `CreateMultiPathGatewaySecretKey` API；如果是 Update 操作，调用 `ModifyMultiPathGatewaySecretKey` API。
