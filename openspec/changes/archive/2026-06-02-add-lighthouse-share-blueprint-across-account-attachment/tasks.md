...
- [x] 1.2 Define Schema with `blueprint_id` (Required, ForceNew, TypeString) and `account_ids` (Required, ForceNew, TypeList of TypeString)
- [x] 1.3 Implement Create function: call `ShareBlueprintAcrossAccounts` API with retry, set composite ID from `blueprint_id` + sorted `account_ids`
- [x] 1.4 Implement Read function: parse composite ID, call `DescribeBlueprintsShareAcrossAccountInfos` API, update or clear state
- [x] 1.5 Implement Delete function: parse composite ID, call `CancelShareBlueprintAcrossAccounts` API with retry
- [x] 1.6 Add `Importer` with `schema.ImportStatePassthrough`
...
- [x] 3.4 Write unit test for Delete scenario using gomonkey to mock `CancelShareBlueprintAcrossAccounts` API
- [x] 3.5 Write unit test for Import scenario
- [x] 3.6 Run unit tests with `go test -gcflags=all=-l` and ensure all tests pass
