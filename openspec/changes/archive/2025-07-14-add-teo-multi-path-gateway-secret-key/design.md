...
- 另有 `CreateMultiPathGatewaySecretKey` 和 `RefreshMultiPathGatewaySecretKey` 接口，但本次需求仅涉及 Read 和 Update 操作
...
**Non-Goals:**
- 不实现 `CreateMultiPathGatewaySecretKey` 和 `RefreshMultiPathGatewaySecretKey` 接口的调用（本次需求仅涉及 Describe 和 Modify）
- 不实现数据源（data source）
- 不实现异步轮询机制（Describe 和 Modify 均为同步接口）

## Decisions

1. **资源 ID 设计**：使用 `zone_id` 作为资源 ID。因为每个站点只有一个密钥配置，zone_id 可以唯一标识一个密钥配置。zone_id 设置为 ForceNew，变更 zone_id 时需要重建资源。

2. **Create 方法设计**：采用 RESOURCE_KIND_CONFIG 的标准模式，Create 方法设置 zone_id 为资源 ID，然后直接调用 Update 方法（即 ModifyMultiPathGatewaySecretKey）。这与 `tencentcloud_teo_ddos_protection_config` 的模式一致。
...
6. **重试机制**：Read 和 Update 操作均使用 `tccommon.ReadRetryTimeout` 进行重试，失败时使用 `tccommon.RetryError()` 包装错误返回。
