...
- 支持通过 CreateSecurityAPIResource 接口创建 API 资源
- 支持通过 DescribeSecurityAPIResource 接口查询 API 资源（含分页）
- 支持通过 ModifySecurityAPIResource 接口修改 API 资源
...
### New Capabilities
- `teo-security-api-resource`: 管理 TEO 站点下的 API 安全资源配置，支持完整的 CRUD 生命周期管理
...
- 修改文件: `tencentcloud/services/teo/service_tencentcloud_teo.go` (新增 DescribeSecurityAPIResourceById 服务方法)
