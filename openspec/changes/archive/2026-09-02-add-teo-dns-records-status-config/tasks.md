...
- [x] 1.2 定义 schema 字段：`zone_id`（Required, ForceNew, TypeString）、`filters`（Optional, TypeList，元素含 `name` Required TypeString、`values` Required TypeList of TypeString、`fuzzy` Optional TypeBool）、`sort_by`（Optional, TypeString）、`sort_order`（Optional, TypeString）、`match`（Optional, TypeString）、`records_to_enable`（Optional, TypeList of TypeString）、`records_to_disable`（Optional, TypeList of TypeString）、`dns_records`（Computed, TypeList，元素含 `zone_id`/`record_id`/`name`/`type`/`location`/`content`/`ttl`/`weight`/`priority`/`status`/`created_on`/`modified_on`）
- [x] 1.3 不暴露 `limit`/`offset` 分页参数；`dns_records` 列表展开平铺，不嵌套 `xxx_set`/`xxx_list` 结构
...
- [x] 2.5 调用成功后，在 retry 块外设置 `d.SetId(zoneId)`，并调用 `resourceTencentCloudTeoDnsRecordsStatusRead(d, meta)` 回填状态
...
- [x] 3.2 填充 `ZoneId`、`Filters`（从 schema `filters` 构建 `[]*AdvancedFilter`，每个含 `Name`/`Values`/`Fuzzy`）、`SortBy`（若有）、`SortOrder`（若有）、`Match`（若有）
...
- [x] 3.4 解析 `response.Response.DnsRecords`，将列表展开平铺到 `dns_records` schema，每个元素字段 set 前判断 nil
- [x] 3.5 若云 API 返回空（response nil / response.Response nil / len(DnsRecords)==0），先 `log.Printf("[CRUD] teo_dns_records_status id=%s", d.Id())` 保留现场，再 `d.SetId("")`
...
- [x] 4.4 因 `ModifyDnsRecordsStatus` 为异步接口，调用成功后调用 `DescribeDnsRecords`（通过 Read）轮询，直到目标记录 `status` 达到期望值（enable/disable）或 `d.Timeout(schema.TimeoutUpdate)` 超时
...
- [x] 7.1 新增 `tencentcloud/services/teo/resource_tc_teo_dns_records_status.md`，包含一句话描述（提及 TEO/EdgeOne）、Example Usage（展示 `zone_id`、`filters`、`records_to_enable`/`records_to_disable`）、Import 部分（说明使用 `zone_id` 作为 ID）
...
- [x] 8.3 新增 Read 测试用例，mock `DescribeDnsRecordsWithContext` 返回 DNS 记录，验证 `dns_records` 字段回填
- [x] 8.4 新增 Update 测试用例，mock `ModifyDnsRecordsStatusWithContext` 和 `DescribeDnsRecordsWithContext`，验证状态更新与轮询
- [x] 8.5 保证生成的代码在当前环境下可正确构建执行（不执行 `go test`，仅保证代码正确性）
