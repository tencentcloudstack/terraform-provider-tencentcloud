...
The resource `tencentcloud_teo_edge_k_v_put` SHALL define the following schema:
...
- **WHEN** user defines a `tencentcloud_teo_edge_k_v_put` resource in HCL
...
- **WHEN** user runs `terraform import tencentcloud_teo_edge_k_v_put.example zone_id#namespace#key`
...
- **WHEN** user references `tencentcloud_teo_edge_k_v_put` in their configuration
- **THEN** Terraform SHALL recognize it as a valid resource type

### Requirement: Unit tests with gomonkey mock

The resource SHALL have unit tests in `resource_tc_teo_edge_k_v_put_attachment_test.go` that use gomonkey to mock the cloud API calls. Tests SHALL cover Create, Read, and Delete operations.
