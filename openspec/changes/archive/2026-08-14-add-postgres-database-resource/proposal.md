...
- Add new resource `tencentcloud_postgres_database` (RESOURCE_KIND_GENERAL) that manages the full lifecycle of a PostgreSQL database within a DB instance.
...
- Add documentation file `resource_tc_postgres_database.md`.
- Add unit tests using gomonkey mock (no TF_ACC test suite).

## Capabilities

### New Capabilities
- `postgres-database-resource`: Resource to manage a PostgreSQL database within a DB instance, supporting create (CreateDatabase), read (DescribeDatabases), update (ModifyDatabaseOwner for owner changes), and delete (DeleteDatabase) operations.
...
- **New files**:
  - `tencentcloud/resource_tc_postgres_database.go` — resource schema and CRUD logic
  - `tencentcloud/resource_tc_postgres_database_test.go` — unit tests with gomonkey mock
  - `tencentcloud/resource_tc_postgres_database.md` — documentation
- **Modified files**:
  - `tencentcloud/provider.go` — register `tencentcloud_postgres_database` resource
